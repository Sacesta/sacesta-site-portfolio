...
function Marker({
  ...
  const surfacePosition = useMemo(() => {
    return latLngToVector3(marker.lat, marker.lng, radius * 1.02);
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      {/* Circular initial bubble directly on the surface */}
      <group ref={imageGroupRef} position={surfacePosition}>
        <Html
          center
          distanceFactor={10}
          zIndexRange={[100, 0]}
...
function RotatingGlobe({
  ...
  // Configure textures (using clone to avoid hook mutation warning)
  const [eTex, bTex] = useMemo(() => {
    if (!earthTexture || !bumpTexture) return [earthTexture, bumpTexture];
